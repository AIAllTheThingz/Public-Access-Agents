---
id: TOOL-PKG-CHECK-LINKS-001
title: Check Links Tool
version: 1.0.0
status: baseline
---
...
Validate relative Markdown file targets and local heading anchors without fetching external URLs.
...
```bash
python tools/check-links/check_links.py --help
```
...
```bash
python tools/check-links/check_links.py --no-check-anchors
```
...
```bash
python -m unittest discover -s tools/tests -p "test_check_links*.py"
```
...
- does not verify external URLs
- does not parse every possible Markdown extension
- does not execute rendered HTML
